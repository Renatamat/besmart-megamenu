...
{function name="categories_links" nodes=[] level=1}
    {strip}
        <ul class="{if $level==1}cbp-links cbp-category-tree ab {elseif $level==2}cbp-hrsub-level2{elseif $level==3}cbp-hrsub-level2 cbp-hrsub-level3{/if}">
            {foreach $categories as $category}
                {if isset($category.title)}
                    <li {if isset($category.children)} class="{if $level==1}cbp-hrsub-haslevel2{else}cbp-hrsub-haslevel3{/if}" {/if}>
                        <div class="cbp-category-link-w inside-submenu">
                            <a href="{$category.href}"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="12" viewBox="0 0 24 12" fill="none">
                                    <path d="M17.9718 1.40625L22.5656 6M22.5656 6L17.9718 10.5938M22.5656 6H1.43433" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                  </svg> 
                            <span class="cbp-prod-title"> {$category.title} {if isset($category.prod_count)}<span class="cbp-prod-count">{$category.prod_count}</span>{/if} </span></a>
            ...
            {if $node.type==2}
                ...
                {elseif $node.contentType==2}
                    ...
                    {if isset($node.content.ids)}
                                    ...
                                    {if isset($category.title)}
                                        <div class="col-{$node.content.line}">
                                            <div class="cbp-category-link-w "><a href="{$category.href}"
                                                                                class="cbp-column-title nav-link cbp-category-title">{if isset($category.prod_count)} <span class="cbp-prod-count-top">{$category.prod_count}</span>{/if}{$category.title}</a>
                        ...
                        {else}
                            <ul class="cbp-links cbp-category-tree THA">
                                {foreach from=$node.content.ids item=category}
                                    {if isset($category.title)}
                ...
                {elseif $node.contentType==3}
                    ...
                    {if isset($node.content.ids)}
                            <ul class="cbp-links cbp-valinks{if !$node.content.view} cbp-valinks-vertical{/if}{if $node.content.view==2} cbp-valinks-vertical cbp-valinks-vertical2{/if}">
                                {foreach from=$node.content.ids item=va_link name=links}
                                    {if isset($va_link.href) && isset($va_link.title) && $va_link.href != '' && $va_link.title != ''}
                                        <li class="cbp-link-item cbp-link-{$smarty.foreach.links.iteration}">
                                            <a href="{$va_link.href}"
                                               {if isset($va_link.new_window) && $va_link.new_window}target="_blank" rel="noopener noreferrer"{/if}>
                                               {$va_link.title}
                                            </a>
                                        </li>
                                    {/if}
