...
');
...

    {
        ...
        foreach ($tabs as $key => $tab) {
            ...
            if (isset($tab['id_tab']) && (int) $tab['id_tab'] === 9) {
                $categoryId = 2;
                $category = new Category((int) $categoryId, $id_lang, $id_shop);
                
                $total = (int) $category->getProducts($id_lang, 1, 1, null, null, true);
                $tabs[$key]['prod_count'] = $total;
            }
        }
    ...
    private function generateCategoriesMenu(
        ...
        foreach ($categories as $key => $category) {
            ...
            if ($subcats) {
                ...
                $return_categories['prod_count'] = $this->countAvailableProductsInCategory((int) $category['id_category']);
    ...
    private function countAvailableProductsInCategory($id_category) {
//        $id_shop = (int) $this->context->shop->id;

//        $sql = 'SELECT COUNT(DISTINCT cp.id_product)
//            FROM `' . _DB_PREFIX_ . 'category_product` cp
//            INNER JOIN `' . _DB_PREFIX_ . 'product_shop` ps
//                ON (ps.id_product = cp.id_product AND ps.id_shop = ' . (int) $id_shop . ' AND ps.active = 1 AND ps.visibility IN ("both","catalog"))
//            INNER JOIN `' . _DB_PREFIX_ . 'stock_available` sa
//                ON (sa.id_product = cp.id_product AND sa.id_product_attribute = 0 AND (sa.id_shop = ' . (int) $id_shop . ' OR sa.id_shop = 0))
...
//        return (int) Db::getInstance()->getValue($sql);
    ...
    // Zwraca COUNT zgodny z logiką Presty (aktywność, widoczność, multishop, access)
    $total = (int) $category->getProducts(
        $id_lang,
        0,  
        0,    
        null, 
